...
rules.add()
        .name("Ship LoRaWAN GNSS → Location")
        .when({
            facts ->
                def updates = []
                facts.matchAssetState(
                        new AssetQuery()
                                .types(ShipTrackerAsset)
                    ...
                    try {
                        ...
                        LOG.warning("Ship rule error [${assetState.id}]: ${e.message}")
                    }
                }
        ...
        .then({
            facts ->
                facts.bound("updates").each { u ->
                    ...
                    if (u.gnssLat != null) LOG.info("Ship ${id} → [${u.gnssLat}, ${u.gnssLon}] bat=${u.battery}%")
                }
        })
